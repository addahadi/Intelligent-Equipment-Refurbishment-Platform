...
export interface Offre { id: number; designation: string; typePropose: TypeComposant; marque: string; modele: string; reference: string; categorieId: number; etatDeclare: EtatDeclare; prixPropose: number; quantite: number; quantiteAcceptee?: number; description?: string; images: string[]; dateOffre: string; statut: StatutOffre; entrepriseId: number; composantId?: number; entreprise?: { id?: number; nom: string; contact: string; adresse?: string } }
export interface Commande { id: number; clientId: number; composantId: number; date: string; prix: number; statut: StatutCommande; dateFinGarantie: string; client?: { nom: string; email: string }; composant?: { nom: string; reference: string; garantie: number } }
